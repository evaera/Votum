...
export default class ArchiveCommand extends Command {
  ...
  async execute (msg: CommandMessage, args: any): Promise<Message | Message[]> {
    ...
    if (raw.length === 0) {
      return msg.reply('The archive command allows you to view past motions in your council. To browse a range of motions, use `!archive #5-#15`. To view a single motion, use `!archive #6`.\n\nThe most recent motion in this council is #' + this.council.numMotions + '.')
    }
    ...
    if (range.length > 2) {
      return msg.reply('Please only provide a range of two numbers, like `#1-#10`.')
    } else if (range.length === 2) {
      if (range[1] <= range[0]) {
        return msg.reply('Second number in range must be greater than the first.')
      }
      ...
      return msg.reply({ embed: {
        ...
        footer: {
          text: 'To view a full motion, run `!archive #1` where `1` is the number of the motion you want to view.'
        },
        color: 0xfaff72
      }})
    } else {
